...
dataset = 'microarray_rna'

train_loaders = [LoadDataset(dataset, data_root, batch_size, 'train', style=domains[0])]
train_loaders += [cycle(LoadDataset(dataset,data_root,batch_size,'train',style=domain)) for domain in domains[1:]] 


test_loaders = [LoadDataset(dataset,data_root,batch_size,'test',style=domain) for domain in domains] 


# Load Model
enc_dim = conf['model']['vae']['encoder'][-1][1] #latent space dimension #100
code_dim = conf['model']['vae']['code_dim'] #number of domains #currently 16 
...
vae = LoadModel('vae',conf['model']['vae'],img_size,img_depth) #img_size=64, img_depth = 16. Last arg is input dimension
d_feat = LoadModel('nn',conf['model']['D_feat'],img_size,enc_dim) #img_size=64, enc_dim=1024
d_pix = LoadModel('nn',conf['model']['D_pix'],img_size,img_depth) #img_size=64, img_depth = 16
...
while global_step < trainer_conf['total_step']: 
    ...
    for train_images in zip(*train_loaders):
        ...
        if trainer_conf['save_log'] and (global_step % trainer_conf['verbose_step'] ==0):
            ...
            writer.add_scalars('Pixel_Distance',{'real':pix_real_pred.item(),
                                               'fake':pix_fake_pred.item()}, global_step)
            writer.add_scalars('Code_loss',{'feature':df_loss.item(),
                                            'pixel':pix_code_loss.item(),
                                           'adv_feature':feature_loss.item(),
                                           'adv_pixel':pix_clf_loss.item()}, global_step)
        ...
        if global_step%trainer_conf['checkpoint_step']==0 and trainer_conf['save_checkpoint'] and not trainer_conf['save_best_only']:
            torch.save(vae.state_dict(),model_path.format(global_step)+'.vae')
